    ...
    with psycopg.connect(database_url) as conn:
        ...
        with open(csv_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                conn.execute(
                    """
                    INSERT INTO public.linkedin_jobs_raw (link_id, job_url, html_object_key, snapshot_object_key, scrape_status)
                    VALUES (%s, %s, %s, %s, 'done')
                    ON CONFLICT (link_id) DO UPDATE
                      SET html_object_key = EXCLUDED.html_object_key,
                          snapshot_object_key = EXCLUDED.snapshot_object_key,
                          scraped_at = now(),
                          scrape_status = 'done'
                    """,
                    (link_id, url, html_path, shot_path),
                )
